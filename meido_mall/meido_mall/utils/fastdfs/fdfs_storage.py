...
class FastDFSStorage(Storage):
    ...
    def _save(self, name, content):
        ...
        """
        # 1。创建fdfs客户端
        # client = Fdfs_client('meido_mall/utils/fastdfs/client.conf')
        # client = Fdfs_client(setting.FDFS_CLIENT_CONF)
        client = Fdfs_client(self.client_conf)

        # 2.上传文件
        # client.upload_by_filename() #如果是指定一个文件路径和文件名来上传文件用此方法 filename上传的图片文件有后缀
        ret = client.upload_by_buffer(content.read()) #如果是通过文件数据的二进制来上传用buffer 上传的图片在storage没有后缀
    ...
    def url(self, name):
        ...
        """
        # return 'http://192.168.133.222:8888/' + name
