...
class CartView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user = request.user # 用于登录用户   首次获取还会做认证
            # 如果代码能继续向下走说明是登录用户存储购物车数据到redis
            """
            cart_user_id: {sku_id_16: 1} hash
            hincrby(cart_1, sku_id_16, 2) # 如果sku_id已存在，直接会做增量计算
            ...
            # 判断本次添加的商品是否在购物车中已存在，如果已存在要做增量计算
    ...
    def get(self, request):
        ...
        try:
            ...
            # 如果获取user出现异常说明当前是未登录用户(获取cookie购物车书库)
            user = None
        else:
            # 如果获取到user说明是已登录用户(操作redis数据库)
            # 创建redis连接对象
    ...
    def put(self, request):
        ...
        try:
            ...
            # 已登录用户操作redis购物车数据
            redis__conn = get_redis_connection('cart')
            pl = redis__conn.pipeline()
